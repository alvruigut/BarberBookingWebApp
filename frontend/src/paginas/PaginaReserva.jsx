...
import { ComprobacionTurnstile } from '../componentes/comunes/ComprobacionTurnstile';
...
export function PaginaReserva() {
  ...
  const enviar = async (evento) => {
    ...
    if (!tokenTurnstile) { setMensaje({ mensaje: 'Espera un momento mientras completamos la verificación de seguridad.' }); return; }
    if (!clave.current) clave.current = nuevaClaveIdempotencia(); setEnviando(true); setMensaje(null);
    try {
      const respuesta = await crearCita(slug, { nombreCliente: datos.nombreCliente.trim(), telefonoCliente: datos.telefonoCliente.trim(), servicioId: Number(datos.servicioId), profesionalId: Number(datos.profesionalId), fechaInicio: datos.fechaInicio, notaCliente: datos.notaCliente.trim() || null }, clave.current, tokenTurnstile);
    ...
    } finally { setEnviando(false); setTokenTurnstile(''); setReinicioTurnstile((valor) => valor + 1); }
  };
  ...
  return <section className="seccion seccion-formulario reserva-moderna">
    ...
    <form className="formulario-reserva-abierto" onSubmit={enviar} noValidate>
      ...
      <ComprobacionTurnstile onCambio={setTokenTurnstile} reinicio={reinicioTurnstile} />
      <button className="boton boton-ancho bloque-ancho-reserva" disabled={enviando || !tokenTurnstile}>{enviando ? 'Confirmando reserva…' : tokenTurnstile ? 'Confirmar cita' : 'Comprobando seguridad…'}</button>
    </form>
  </section>;
}
