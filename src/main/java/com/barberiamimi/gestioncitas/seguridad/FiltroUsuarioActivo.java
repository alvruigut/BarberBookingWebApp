...
public class FiltroUsuarioActivo extends OncePerRequestFilter {
    ...
    public FiltroUsuarioActivo(ObjectProvider<RepositorioUsuarioAdministracion> proveedorUsuarios, ObjectMapper json) {
        this.proveedorUsuarios = proveedorUsuarios;
        this.json = json;
    }
    ...
    protected void doFilterInternal(HttpServletRequest solicitud, HttpServletResponse respuesta, FilterChain cadena)
        ...
        if (usuarios != null && solicitud.getRequestURI().startsWith("/api/administracion/") && autenticacion != null
                && autenticacion.getPrincipal() instanceof UsuarioAutenticado usuario
                && !usuarios.estaActivo(usuario.getId(), usuario.getBarberiaId())) {
            if (solicitud.getSession(false) != null)
                solicitud.getSession(false).invalidate();
            SecurityContextHolder.clearContext();
            respuesta.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
            respuesta.setContentType(MediaType.APPLICATION_JSON_VALUE);
            json.writeValue(respuesta.getOutputStream(),
                    new ErrorRespuesta("USUARIO_INACTIVO", "El acceso de este empleado está deshabilitado.", List.of(),
                            LocalDateTime.now(), solicitud.getRequestURI()));
            return;
        }
