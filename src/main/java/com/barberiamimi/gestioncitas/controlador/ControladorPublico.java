...
public class ControladorPublico {
    ...
    public ResponseEntity<CitaCreadaRespuesta> crear(@PathVariable String slug,@Parameter(description="Clave única del intento de reserva",required=true) @RequestHeader("Idempotency-Key") String clave,@Parameter(description="Token de verificación de Cloudflare Turnstile",required=true) @RequestHeader("Turnstile-Token") String tokenTurnstile,@Valid @RequestBody SolicitudCrearCita solicitud,HttpServletRequest peticion){proteccion.validarIntento(slug,tokenTurnstile,obtenerIp(peticion));CitaCreadaRespuesta r=citas.crearPublica(slug,clave,solicitud);return ResponseEntity.status(r.repetida()?HttpStatus.OK:HttpStatus.CREATED).body(r);}
