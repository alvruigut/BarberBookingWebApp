...
public class ConfiguracionSeguridad {
    ...
    @Bean
    AuthenticationManager gestorAutenticacion(AuthenticationConfiguration c) throws Exception {
    ...
    SecurityFilterChain cadena(HttpSecurity http, ObjectMapper json, FiltroUsuarioActivo filtroUsuarioActivo)
        ...
        http.cors(c -> {
                ...
                .authorizeHttpRequests(a -> a
                        ...
                        .permitAll().requestMatchers("/api/administracion/**").hasAnyRole("PROPIETARIO", "BARBERO")
                        .anyRequest().authenticated())
                .sessionManagement(s -> s.sessionFixation(f -> f.changeSessionId()).maximumSessions(2))
                .exceptionHandling(e -> e
                        .authenticationEntryPoint((req, res, ex) -> escribirError(res, json, 401, "NO_AUTENTICADO",
                                "Es necesario iniciar sesión.", req.getRequestURI()))
                        .accessDeniedHandler((req, res, ex) -> escribirError(res, json, 403, "ACCESO_DENEGADO",
    ...
    private static void escribirError(HttpServletResponse r, ObjectMapper json, int estado, String codigo,
            String mensaje, String ruta) throws java.io.IOException {
        r.setStatus(estado);
        r.setContentType(MediaType.APPLICATION_JSON_VALUE);
        json.writeValue(r.getOutputStream(), new ErrorRespuesta(codigo, mensaje, List.of(), LocalDateTime.now(), ruta));
    }
}
